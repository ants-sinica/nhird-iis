\ |IMG1|\ 

.. _h106d6a60386b4471802c17574203f54:

健康雲巨量資訊技術平台
**********************

中央研究院資訊科學研究所


.. toctree:: 
    :maxdepth: 2

    Accounts
    執行查詢
    工作區
    資料集

.. _h174fb648377959437b5c1f697c1c40:

計畫目標
========

.. _h174fb648377959437b5c1f697c1c40:

計畫概況
--------

* \ `使用方式簡介`_\ 
* \ `申請帳號`_\ 
* \ `網站功能`_\ 
* \ `資料集`_\ 
* \ `功能拼圖`_\ 
* \ `簡稱功能`_\ 


.. _`使用方式簡介`: http://nhird-iis.readthedocs.io/en/latest/tutorial.html
.. _`申請帳號`: http://nhird-iis.readthedocs.io/en/latest/apply.html
.. _`網站功能`: http://nhird-iis.readthedocs.io/en/latest/web.html
.. _`資料集`: http://nhird-iis.readthedocs.io/en/latest/dataset.html
.. _`功能拼圖`: http://nhird-iis.readthedocs.io/en/latest/blocks.html
.. _`簡稱功能`: http://nhird-iis.readthedocs.io/en/latest/shortcuts.html

.. |IMG1| image:: static/index_1.png
   :height: 82 px
   :width: 82 px
